    ...
    return (
        isShowing ? ReactDOM.createPortal(
            <>
                ...
                <div
                    className="z-30 bg-black overflow-hidden relative flex justify-center items-center max-w-40 mx-auto my-60 rounded-xl h-2/5 p-2 shadow-md
                ...
                >
                    ...
                    <div className={`${!confirmed ? 'translate-x-full' : ''} absolute flex flex-col justify-center items-center px-2 text-center`}>
                        <span className="text-2xl text-cyan-500 font-bold uppercase">Đã gửi form</span>
                        <span className="text-lg font-bold mb-2">Tuyệt vời! Cảm ơn bạn rất nhiều</span>
                        <p className="text-sm">Chúng mình sẽ liên hệ lại với bạn để xác nhận lại đơn hàng nhé</p>
                        <span className="iconify text-cyan-500 my-6" data-icon="charm:circle-tick" data-width="80" data-height="80"></span>
                        <button 
                            onClick={hide}
                            onBlur={()=> setConfirmed(false)} 
                            className="w-44 border-2 border-cyan-500 rounded-md shadow-md mx-1 my-2 px-6 py-3 font-bold overflow-hidden cursor-pointer"
                        >
                            <Link href="/"><a>Về trang chủ</a></Link>
                        </button>
                    </div>
                </div>
                </div>
